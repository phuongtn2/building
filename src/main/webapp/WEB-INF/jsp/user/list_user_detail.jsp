				...
				<table class="footable emp-sales table table-stripped table-bordered table-hover dataTables-example"
					...
					<c:forEach items="${userDtoList}" var="user">
						<tr class="gradeC">
							...
							<td data-hide="phone,tablet">
								<c:if test="${user.userStatus==1}"> <span
										class="label label-success">Active</span> </c:if>
								<c:if test="${user.userStatus==2}"> <span
										class="label label-danger">Deactive</span> </c:if>
								<c:if test="${user.userStatus==3}"> <span
										class="label label-primary">Pending</span> </c:if>
							</td>
							<td data-hide="phone,tablet"><fmt:formatDate pattern="MM/dd/yyyy"
							...
							<td class="text-center">
								<div class="btn-group">
									...
									<a type="button" data-toggle="modal" data-target="#deleteButton"
									   class="btn-danger btn btn-xs" onclick="setUrl('/user/delete/${user.userId}')">Delete</a>
								</div>
							</td>
						</tr>
					</c:forEach>
