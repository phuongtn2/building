					...
					<c:forEach items="${userDtoList}" var="user">
						<tr class="gradeC">
							...
							<td data-hide="phone,tablet">
								<c:if test="${user.userStatus==1}"> <span class="label label-success"><spring:message
										code="common.active" text="default text"/></span> </c:if>
								<c:if test="${user.userStatus==2}"> <span class="label label-danger"><spring:message
										code="common.deactive" text="default text"/></span> </c:if>
								<c:if test="${user.userStatus==3}"> <span class="label label-primary"><spring:message
										code="common.pending" text="default text"/></span> </c:if>
							</td>
							<td data-hide="phone,tablet"><fmt:formatDate pattern="MM/dd/yyyy"
							                                             value="${user.startDay}"/></td>
							<td data-hide="phone,tablet"><fmt:formatDate pattern="MM/dd/yyyy"
							                                             value="${user.endDay}"/></td>
							<td data-hide="phone,tablet">${user.memo}</td>
							<td class="text-center">
								<div class="btn-group">
									<a class="btn-success btn btn-xs" href="/user/edit/${user.userId}"><spring:message
									...
									<a type="button" data-toggle="modal" data-target="#deleteButton"
									   ...
									   onclick="setUrl('/user/delete/${user.userId}')"><spring:message
											code="common.button.delete" text="default text"/></a>
									<a class="btn-success btn btn-xs" href="/user/view/${user.userId}"><spring:message
