					...
					<div class="row">
						<div class="col-sm-8">
							<div class="form-group">
								<label class="control-label" for="roomAlias"><spring:message
										code="building.room.roomName" text="default text"></spring:message></label>
								<input type="hidden" id="roomCode" name="roomCode"
								       value="<c:if test="${roomDto.roomCode!= null}">${roomDto.roomCode}</c:if>"
								       class="form-control">
								<input type="text" id="roomAlias" name="roomAlias"
								       value="<c:if test="${roomDto.roomAlias!= null}">${roomDto.roomAlias}</c:if>"
						...
						<div class="col-sm-2">
							<div class="form-group">
								<label class="control-label" for="count"><spring:message
										code="building.room.countDetail" text="default text"></spring:message></label>
								<input type="text" id="count" name="count"
								       value="<c:if test="${roomDto.count!= 0}">${roomDto.count}</c:if>"
								       placeholder="count" class="form-control">
							</div>
						</div>
						<div class="col-sm-2">
							<div class="form-group">
								<label class="control-label"><spring:message code="building.room.status"
								                                             text="default text"></spring:message></label>
								<select name="status" class="form-control m-b">
									<option
											<c:if test="${roomDto.status==1}">selected</c:if> value="1">Phòng trống
									</option>
									<option
											<c:if test="${roomDto.status==2}">selected</c:if> value="2">Đã thuê
									</option>
								</select>
							</div>
						</div>
